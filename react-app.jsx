    ...
    try {
      ...
      return arr.map(i => {
        if (i && typeof i === 'object') {
          const createdAt = (i.createdAt != null) ? i.createdAt : Date.now();
          const completedAt = i.completed ? ((i.completedAt != null) ? i.completedAt : Date.now()) : null;
          ...
          <>
            ...
            {item.editedAt ? (
              <>
                ...
                <time
                  className="meta"
                  dateTime={new Date(item.editedAt).toISOString()}
                  title={formatDateTime(item.editedAt)}
                >
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo){ try { console.error('React error:', error, errorInfo); } catch(e){} }
  render(){
    if (this.state && this.state.error) {
      const msg = (this.state.error && this.state.error.message) ? this.state.error.message : String(this.state.error);
      return (
        <div style={{ padding: '16px', color: '#e8eef5' }}>
          <h2>Ocorreu um erro</h2>
          <pre style={{ whiteSpace: 'pre-wrap', color: '#ffb4b4' }}>{msg}</pre>
        </div>
      );
    }
